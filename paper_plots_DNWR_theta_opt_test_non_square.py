...
def run_all(output_file, s, N_steps = 100, n1 = 50, n2 = 32, thmin = 0, thmax = 1, **kwargs):
    ...
    res_IE_1D = get_conv_rates(N_steps = N_steps, s = s, order = 1, dim = 1, n = n1, init_cond = get_init_cond(1, True), thmin = thmin, thmax = thmax, **kwargs)
    res_IE_2D = get_conv_rates(N_steps = N_steps, s = s, order = 1, dim = 2, n = n2, init_cond = get_init_cond(2, True), thmin = thmin, thmax = thmax, **kwargs)
    res_S2_1D = get_conv_rates(N_steps = N_steps, s = s, order = 2, dim = 1, n = n1, init_cond = get_init_cond(1, True), thmin = thmin, thmax = thmax, **kwargs)
    res_S2_2D = get_conv_rates(N_steps = N_steps, s = s, order = 2, dim = 2, n = n2, init_cond = get_init_cond(2, True), thmin = thmin, thmax = thmax, **kwargs)
...
def plotting(input_file, savefile):
    ...
    conv_rates_theo = [p.theoretical_conv_rate(dt, th) for th in thetas]
