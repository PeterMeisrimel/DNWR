...
def get_solver(prob, order, WR_type = 'DNWR'):
    ...
    if WR_type == 'DNWR':
        ...
        elif order == -2: return prob.NNWR_SDIRK2_TA_single ## should be double, but double is deprecated
...
def get_init_cond(dim, extra_len = False, num = 1):
    if extra_len:
        if num == 1:
            if dim == 1:   return lambda x: 500*sin(pi/10*(x+9))
            elif dim == 2: return lambda x, y: 500*sin(pi*y)*sin((pi/10)*(x + 9))
        elif num == 2:
...
def verify_space_error(tf = 1, init_cond = None, n_min = 2, N_steps = 100, k = 8, ex_sol = None, savefig = None, **kwargs):
    ...
    for n in n_list:
        ...
        u1, u2, ug, flux, prob = solve_monolithic(tf, N_steps, init_cond, order = 2, n = n, **kwargs)
    ...
    if savefig is not None:
        s = 'err_space_tf_{}_steps_{}_dim_{}.png'.format(tf, N_steps, kwargs['dim'])
...
def verify_mono_time(tf = 1, init_cond = None, order = 1, k = 5, savefig = None, **kwargs):
    u1_ref, u2_ref, ug_ref, _, prob = solve_monolithic(tf, 2**(k+1), init_cond, order, **kwargs)
    ...
    if savefig is not None:
        s = 'mono_time_tf_{}_ord_{}_dim_{}.png'.format(tf, order, kwargs['dim'])
        pl.savefig(savefig + s, dpi = 100)
...
def verify_with_monolithic(tf = 1, N_steps = 20, init_cond = None, order = 1, k = 10, theta = None, WR_type = 'DNWR', savefig = None, **kwargs):
    ...
    if WR_type == 'NNWR' and ID_SELF == 1:
        ...
        u1_ref, u2_ref, ug_ref, _, prob_mono = solve_monolithic(tf, N_steps, init_cond, order, **kwargs)
    ...
    pl.grid(True, which = 'major')
    if savefig is not None:
        s = 'verify_mono_time_steps_{}_dim_{}_ord_{}.png'.format(N_steps, kwargs['dim'], order)
        pl.savefig(savefig + s, dpi = 100)
    ...
    pl.grid(True, which = 'major')
    if savefig is not None:
        s = 'verify_mono_time_iter_steps_{}_dim_{}_ord_{}.png'.format(N_steps, kwargs['dim'], order)
        pl.savefig(savefig + s, dpi = 100)
...
def verify_comb_error(init_cond, tf = 1, k = 10, kmin = 0, theta = None, WR_type = 'DNWR', order = 1, savefig = None, **kwargs):
    ...
    if WR_type == 'NNWR' and ID_SELF == 1:
        ...
        u1_ref, u2_ref, ug_ref, _, prob_mono = solve_monolithic(tf, 2**(k+1), init_cond, order, **kwargs)
    ...
    pl.grid(True, which = 'major')
    if savefig is not None:
        s = 'verify_comb_error_iters_dim_{}_ord_{}.png'.format(kwargs['dim'], order)
        pl.savefig(savefig  + s, dpi = 100)
...
def verify_MR_comb(init_cond, tf = 1, k = 10, kmin = 0, theta = None, WR_type = 'DNWR', order = 1, savefig = None, C = 10, TOL = 1e-13, **kwargs):
    ...
    if WR_type == 'NNWR' and ID_SELF == 1:
        ...
        u1_ref, u2_ref, ug_ref, _, prob_mono = solve_monolithic(tf, C*2**(k+1), init_cond, order, **kwargs)
    if WR_type == 'NNWR': comm.Barrier() ## sync, only one process claculates monolithic sol
    ...
    for n_steps in [2**i for i in range(kmin, k)]:
        ...
        ## coarse-coarse
        u1, u2, ug, E, it = solver(tf, n_steps, n_steps, init_cond, theta, TOL = TOL)
        errs_cc['u'].append(prob_mono.norm_inner(u1_ref - u1, u2_ref - u2, ug_ref - ug))
        errs_cc['it'].append(it)
        
        ## coarse-fine
        u1, u2, ug, E, it = solver(tf, n_steps, C*n_steps, init_cond, theta, TOL = TOL)
        errs_cf['u'].append(prob_mono.norm_inner(u1_ref - u1, u2_ref - u2, ug_ref - ug))
        errs_cf['it'].append(it)
        
        ## fine-coarse
        u1, u2, ug, E, it = solver(tf, C*n_steps, n_steps, init_cond, theta, TOL = TOL)
        errs_fc['u'].append(prob_mono.norm_inner(u1_ref - u1, u2_ref - u2, ug_ref - ug))
    ...
    pl.grid(True, which = 'major')
    if savefig is not None:
        s = 'verify_MR_error_dim_{}_ord_{}_C_{}.png'.format(kwargs['dim'], order, C)
        pl.savefig(savefig  + s, dpi = 100)
    ...
    pl.grid(True, which = 'major')
    if savefig is not None:
        s = 'verify_MR_error_iters_dim_{}_ord_{}.png'.format(kwargs['dim'], order)
        pl.savefig(savefig  + s, dpi = 100)
    ...
    pl.legend()
    if savefig is not None:
        s = 'verify_MR_approx_ord_dim_{}_ord_{}.png'.format(kwargs['dim'], order)
        pl.savefig(savefig  + s, dpi = 100)
...
def verify_test(init_cond, tf = 1, k = 10, kmin = 0, theta = None, WR_type = 'DNWR', order = 1, savefig = None, **kwargs):
    ...
    for n_steps in [2**i for i in range(kmin, k)]:
        u1_mono, u2_mono, ug_mono, _, _ = solve_monolithic(tf, n_steps, init_cond, order, **kwargs)
        errs_time.append(prob_mono.norm_inner(u1_ref - u1_mono, u2_ref - u2_mono, ug_ref - ug_mono))
        
        dts.append(tf/n_steps)
        u1, u2, ug, E, it = solver(tf, n_steps, n_steps, init_cond, theta, TOL = 1e-13)
        errs_split.append(prob_mono.norm_inner(u1_mono - u1, u2_mono - u2, ug_mono - ug))
...
def verify_adaptive(init_cond, tf = 1, k_ref = 10, k = 10, WR_type = 'DNWR', which_ref = 'fine', order = -2, savefig = None, **kwargs):
    ...
    if WR_type == 'NNWR' and ID_SELF == 1:
        ...
        if which_ref == 'fixed':
            u1_ref, u2_ref, ug_ref, _, prob_norm = solve_monolithic(tf, 2**k_ref, init_cond, order = order, **kwargs)
        elif which_ref == 'fine':
    ...
    pl.xlabel('TOL'); pl.ylabel('Err')
    if savefig is not None:
        s = 'verify_adaptive_dim_{}_n_{}_order_{}.png'.format(kwargs['dim'], kwargs['n'] , order)
        pl.savefig(savefig + s, dpi = 100)
    ...
    pl.title('Adaptive verification')
    if savefig is not None:
        s = 'verify_adaptive_iters_dim_{}_n_{}_order_{}.png'.format(kwargs['dim'], kwargs['n'] , order)
        pl.savefig(savefig + s, dpi = 100)
    ...
    pl.title('Adaptive verification')
    if savefig is not None:
        s = 'verify_adaptive_timesteps_dim_{}_n_{}_order_{}.png'.format(kwargs['dim'], kwargs['n'] , order)
        pl.savefig(savefig + s, dpi = 100)
