...
def plot_combined(input_file, savefile, hide = False):
    ...
    pl.loglog(res_IE_1D['dts'], (dts/dts[0])**2*(res_S2_1D['u'][0] + res_S2_2D['u'][0])/10, label = '2nd', linestyle = '--')
...
if __name__ == '__main__':
    ...
    for tf, which, C1, C2 in [(1, 'air_water', 10, 1),
                              ...
                              (1, 'water_steel', 1, 10)]:
        add = '_tf_1'
        save_file_data = f'plots_data/NNWR_{which}_{C1}_{C2}_MR_order{add}.txt'
#        run_combined(tf = tf, k1 = 6, k2 = 5, n1 = 9, n2 = 9, C1 = C1, C2 = C2, savefile = save_file_data, **get_parameters(which))
        ...
        plot_combined(save_file_data, f'plots/NNWR_{which}_{C1}_{C2}_MR_order{add}.png')