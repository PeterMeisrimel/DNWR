...
This is the version Azahar described in the NNWR paper
"""
#############################################################
def NNWR_SDIRK2_TA_single(self, tf, init_cond, maxiter = 100, TOL = 1e-8):
    ...
    for k in range(maxiter):
        ...
        j, u1_list = 0, [] # initial flux computation
        while t + dt < tf:
...
def NNWR_SDIRK2_TA_double(self, tf, init_cond, maxiter = 100, TOL = 1e-8, flux0_init = False):
    print('outdated and no longer maintained method')
    if self.WR_type != 'NNWR': raise ValueError('invalid solution method')
    if self.comm.size != 2: raise ValueError('incorrect number of processes, needs to be exactly 2')
    TOL_D = TOL/5 ## paper
...
if __name__ == '__main__':
    ...
    for order in [-1]: # single
        ...
        for which in ['air_water', 'air_steel', 'water_steel']:
            ...
            verify_adaptive(k = 6, savefig = save + which + '/' + which, **p1)
            verify_adaptive(k = 7, savefig = save + which + '/' + which, **p2)